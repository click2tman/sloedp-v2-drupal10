<?php
function sloedp_2023_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) { 
        $size[$i] = $i;
...
function sloedp_2023_render_option_font(){
    $fonts = sloedp_2023_fonts();
    $output = '';
...
function sloedp_2023_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function sloedp_2023_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(sloedp_2023_fonts())) > 17){
...
function sloedp_2023_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= sloedp_2023_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= sloedp_2023_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function sloedp_2023_options_patterns(){
    $output = '';
    $file_path = \Drupal::service('extension.list.theme')->getPath('sloedp_2023');
