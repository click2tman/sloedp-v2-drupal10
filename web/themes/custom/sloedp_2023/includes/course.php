...
function sloedp_2023_check_registered_course($uid, $course_id){
...
function sloedp_2023_preprocess_course(&$vars){
  ...
  $uid = $current_user->id();
  if(sloedp_2023_check_registered_course($uid, $id)){
...
function sloedp_2023_preprocess_commerce_product__course(&$vars){
  $vars = sloedp_2023_preprocess_course($vars);
}
function sloedp_2023_preprocess_commerce_product__course__teaser(&$vars){
  $vars = sloedp_2023_preprocess_course($vars);
}
function sloedp_2023_preprocess_commerce_product__course__teaser_2(&$vars){
  $vars = sloedp_2023_preprocess_course($vars);
}
function sloedp_2023_preprocess_commerce_product__course__featured(&$vars){
  $vars = sloedp_2023_preprocess_course($vars);
}

function sloedp_2023_preprocess_node__lesson(&$variables) {
  $variables['#attached']['library'][] = 'sloedp_2023/gavias-lesson-video';
  ...
  if($node = $variables['node']){
    ...
    if(in_array("administrator", $role)){
      ...
      if($lesson_access == 'registered'){
        if( $user->id() && sloedp_2023_check_registered_course($user->id(), $course_id) ){
          $lesson_icon = 'fa fa-check';
          if(isset($variables['content']['field_lesson_content'])){
            $lesson_content = $variables['content']['field_lesson_content'];
          }    
